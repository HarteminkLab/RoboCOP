    ...
    ) 
{
    ...
    int i;
    for(i = 0; i < idx; i++) {
        sum += table[I(row, i, ncol)];
    }

    for(i = 0; i < ncol; i++) {
        table[I(row, i, ncol)] /= sum;
    }
    ...
    for(i = 1; i < n_obs; i++) {
		...
		for(l = 0; l < n_vars; l++) emissionValues *= emission_mat[I3(l, n_obs - i, n_obs, child, n_states)];
		  ...
		  emissionValues;
                ...
                //fprintf(stderr, "i=%d, j=%d, child=%d, transition=%f,emission=%f\n", i, j, child,transition_mat[I(j, child , n_states)],emission_mat[I(n_obs - i, child, n_states)]);
        ...
        for(j = n_states - 1; j > silent_states_begin - 1; j--) {
	      ...
	      for(l = 0; l < n_vars; l++) emissionValues *= emission_mat[I3(l, n_obs - i - 1, n_obs, child, n_states)];
                ...
                //fprintf(stderr, "i=%d, j=%d, child=%d, transition=%f,emission=%f\n", i, j, child,transition_mat[I(j, child , n_states)],emission_mat[I(n_obs - i - 1, child, n_states)]);
   ...
   for(i = 0; i < n_obs; i++) {
    for(j = 0; j < n_states; j++) {
        posterior_table[I(i,j,n_states)] = 
            sb[i] * sr[i] * f_table[I(i,j,n_states)] * b_table[Ir(i,j,n_obs,n_states)];
    }
   }
}
